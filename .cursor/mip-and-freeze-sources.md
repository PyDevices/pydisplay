...
| `mip.install("github:PyDevices/pydisplay/packages/utils.json")` | GitHub repo manifests (`utils` / `examples` / demos) | **`.py`** (never `.mpy`) |
...
- `packages/*.json` (utils, examples, spibus, i80bus, board bundles)
...
2. **Per-usermod `manifest.py` files** under cmods siblings (e.g.
   [`cmods/pygraphics/manifest.py`](../../cmods/pygraphics/manifest.py)) —
   freeze pure-Python / helper modules for that cmod.
...
The pygraphics cmod ships [`cmods/pygraphics/manifest.py`](../../cmods/pygraphics/manifest.py)
with freezes for the importable `pygraphics` package (plus any `freeze_as_str`
helpers listed there).
