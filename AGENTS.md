...
- The single shared periodic timer is owned by `eventsys.Runtime`
  ...
  `runtime.claim_display_refresh()` (LVGL via `display_driver` from lv_bindings / lvgl-cpython).
...
- `display_driver` (from [lv_bindings](https://github.com/PyDevices/lv_bindings)
  / frozen in MP·CP / bundled with `lvgl-cpython`) owns the LVGL `event_loop`
  (tick via `runtime.on_tick`, `asyncio` from `multimer`) and claims runtime
  display refresh so LVGL presents frames from `task_handler`.
