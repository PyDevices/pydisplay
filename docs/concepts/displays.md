...
SDL2 bindings for **`SDLDisplay`**: native **`usdl2`** when built in or installed from TestPyPI, else pure-Python **`usdl2-py`** / MIP ``usdl2`` from [PyDevices/usdl2](https://github.com/PyDevices/usdl2) (`lib/usdl2/`). Build the native module for frozen MicroPython and CircuitPython Unix — see [MicroPython — usdl2](../platforms/micropython.md#usdl2-native-sdl2).
