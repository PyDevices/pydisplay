...
Use or adapt `tft_config.py` from [`lib/utils/tft_config.py`](https://github.com/PyDevices/pydevices-examples/blob/main/lib/utils/tft_config.py). Some example filenames were search-replaced during port — if an example fails to import config, compare with that file.
