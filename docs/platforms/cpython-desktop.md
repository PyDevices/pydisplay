...
On Windows and macOS, default **`multimer.Timer`** uses a background thread (not SDL timers). Call **`run_scheduled()`** in your main loop so timer callbacks run on the main thread — see [multimer](../concepts/multimer.md).
