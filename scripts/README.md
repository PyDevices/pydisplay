...
|--------|---------|-------------|
| `install_` | `install_gen_manifests.py`, `install_refresh_manifests.sh`, `install_sync_framebuf.py` | `src/` tree changes → updates `packages/*.json`, `web/pyscript/micropython.toml`, `sim/wokwi/mcu-lib.json`; `src/add_ons/framebuf.py` changes → regenerates gitignored `src/lib/graphics/framebuf.py` |
...
| `assets_` | `assets_convert_md_png_to_pbm.py`, `assets_generate_pdwidgets_icons.py`, `assets_make_color_icons.py` | Icon authoring: bulk MD → PBM; curated pdwidgets mono icons; color BMP565 status icons |
...
Describe hardware once under `scripts/board_config/manifests/`, then:
...
Host backends and other exceptions live in `scripts/board_config/hand_maintained/`.
...
See `scripts/board_config/README.md`. E-paper boards live in
`scripts/board_config/manifests/epaperdisplay.toml` and are generated by the same
`generate_board_configs.py` (optionally `--kind epaper`).
