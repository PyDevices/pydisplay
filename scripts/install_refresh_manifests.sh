...
PYSCRIPT_JSON_CONFIGS=(
    ...
    web/pyscript/pydisplay.json
    web/pyscript/peterhinch-nano.json
    web/pyscript/peterhinch-micro.json
    web/pyscript/peterhinch-touch.json
)

# pydisplay core packages (displaydev/eventsys/multimer) are micropython-lib only.
...
pydisplay `eventsys` is not emitted as packages/eventsys.json — install from
the micropython-lib MIP index (`displaydev` / `multimer` / `events` / `keys`
come from micropython-hardware via that same index). Sister packages
...
Not generated here (edit manually):
  ...
  (bus/touch chip MIP packages: sibling micropython-hardware/packages/)
    ...
    for path in "${generated[@]}"; do
        case "$path" in
            ...
            "$PYODIDE_TOML")
                ...
                ;;
            web/pyscript/micropython.json|web/pyscript/pyodide.json|web/pyscript/pydisplay.json|web/pyscript/peterhinch-*.json)
