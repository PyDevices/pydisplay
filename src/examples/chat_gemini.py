# gallery: skip
# deps: lvgl
"""LVGL chat → Gemma (LM Studio) → Gemini speaks the reply.

Chat answers come from Gemma in LM Studio (``CHAT_MODEL``, default
``google/gemma-4-e4b``); Gemini streaming TTS reads them aloud.
...
MAX_CHAT_ATTEMPTS = 2
UI_SETTLE_MS = 350
SPEECH_STYLE = "Read clearly in a friendly, relaxed voice."
...
def _speak(text):
    ...
    stream = None
    try:
        stream = _client.stream(text, voice=voice, instructions=SPEECH_STYLE)
...
def send(text=None):
    """Send the input line: chat completion, then Gemini playback."""
    global _busy
    if _busy:
        return
    ...
    try:
        try:
            ...
            _set_status(msg, "err")
            print("chat_gemini:", msg)
            return
        _add_bubble("assistant", reply)
        # Finish revealing the complete reply before Gemini generation blocks.
        _settle_ui()
        ...
        try:
            ...
            msg = _short_error(exc, "Gemini unreachable")
            _set_status(msg, "err")
            print("chat_gemini speak:", msg)
    finally:
...
try:
    ...
    _status.set_text("Ask a question — %s answers, Gemini speaks" % CHAT_MODEL)
    ...
    _add_bubble(
        "assistant",
        "Hi — ask me anything. I answer with %s, then Gemini reads it aloud." % CHAT_MODEL,
        refresh=False,
    )
finally:
    if _inst is not None:
...
_refresh_ui()
print("chat_gemini: ready — chat=%s speak=Gemini" % CHAT_MODEL)
