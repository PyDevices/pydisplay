...
Setup notes::
    ...
    https://pydevices.github.io/micropython-hardware/audio.html#speech

Voice catalog comes from ``GeminiTTS.voices()`` (documented prebuilt list).
...
def speak(text=None):
    ...
    try:
        ...
        if wait is not None:
            print("tts_gemini: retry in %d s" % wait)
            _start_retry_countdown(wait)
        else:
            msg = _status_for_error(exc)
            _set_status(msg, "err")
            _set_speak_enabled(True)
            print("tts_gemini:", msg)
        return 0
    finally:
        output.close()
...
_refresh_ui()
print("tts_gemini: ready - pick voice/style, then tap Speak")
