# gallery: skip
# deps: lvgl
"""LVGL textarea → Groq-hosted Orpheus TTS → board_config.audio_out.

Needs ``tts``, ``secrets.GROQ_API_KEY``, and ``board_config`` with lazy
``audio_out`` in ``DEVICES``.

API documentation::

    https://console.groq.com/docs/text-to-speech

Voice catalog comes from ``GroqTTS.voices()``. Put bracketed vocal directions
in Style or Text (for example ``[cheerful]`` or ``[whisper]``).
"""
...
DEFAULT_TEXT = "Hello from MicroPython. Groq Orpheus text to speech is working."
...
def _speak_options():
    ...
    if style:
        # Prepended into the spoken text (bracketed vocal direction).
...
def speak(text=None):
    ...
    try:
        ...
        if wait is not None:
            print("tts_groq: retry in %d s" % wait)
...
try:
    ...
    title = lv.label(form)
    title.set_text("Groq Orpheus TTS")
...
_refresh_ui()
print("tts_groq: ready - pick voice/style, then tap Speak")
