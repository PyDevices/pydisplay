# gallery: skip
# deps: lvgl
"""LVGL textarea → local Kokoro TTS → board_config.audio_out.

Needs ``tts``, a reachable Kokoro OpenAI-compatible server
(``secrets.KOKORO_BASE_URL``, default ``http://127.0.0.1:8880/v1``), and
``board_config`` with lazy ``audio_out`` in ``DEVICES``.

Setup (ESP32-P4 board package, host server, ``--http h11``, firewall)::

    https://pydevices.github.io/micropython-hardware/audio.html#local-kokoro-tts

Voice catalog comes from ``KokoroTTS.voices()``. Optional Style is unused by
Kokoro (left for UI parity); leave blank.
"""
...
DEFAULT_TEXT = "Hello from MicroPython. Local Kokoro text to speech is working."
DEFAULT_STYLE = ""
...
def _speak_options():
    ...
    if style:
        # Prepend only when the user typed something (Orpheus tags / stage notes).
...
def _status_for_error(exc):
    ...
    if "connection" in lower or "econn" in lower or "timed out" in lower:
        return "Kokoro server unreachable"
    if len(msg) > 80:
        return msg[:77] + "..."
    return msg
...
def speak(text=None):
    ...
    try:
        ...
        if wait is not None:
            print("tts_kokoro: retry in %d s" % wait)
            _start_retry_countdown(wait)
        else:
            msg = _status_for_error(exc)
            _set_status(msg, "err")
            _set_speak_enabled(True)
            print("tts_kokoro:", msg)
        return 0
    finally:
        output.close()
...
try:
    ...
    _add_label(form, "Style (optional)", body_pt, y, w, pad)
    y += label_h
...
_refresh_ui()
print("tts_kokoro: ready - pick voice/style, then tap Speak")
