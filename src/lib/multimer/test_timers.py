...
from multimer._schedule import REQUIRES_RUN_SCHEDULED, run_scheduled
...
def _run_timer_test(name, TimerClass):
    """
...
def _probe(name, import_fn):
    ...
    try:
        status, detail = _run_timer_test(name, TimerClass)
    except Exception as err:
        print("  FAIL (runtime)")
        _print_error("reason", err)
        try:
...
def main():
    ...
    probes = (
        ("machine.Timer", _import_machine_timer),
        ("_librt.Timer", _import_librt_timer),
        ("_sdl2.Timer", _import_sdl2_timer),
        ("_threading.Timer", _import_threading_timer),
        ("multimer.Timer (default)", _import_multimer_timer),
    )

    for name, import_fn in probes:
        _probe(name, import_fn)
